...
/*描述一个子进程的类，m_pid是目标子进程的PID，m_pipefd是父进程和子进程通讯用的管道*/
class process
{
    public:
        process():m_pid(-1){}
...
static void addsig(int sig, void(handler)(int), bool restart = true)
{
    ...
    assert( -1 != sigaction(sig, &sa, NULL) );
}
...
void processpool<T>::setup_sig_pipe()
{
    ...
    /*设置信号处理函数*/
    addsig(SIGCHLD, sig_handler);     //获取子进程退出时信息
    addsig(SIGTERM, sig_handler);     //定时事件
    addsig(SIGINT, sig_handler);      //程序终止信号
    addsig(SIGPIPE, SIG_IGN);         //往关闭socket里写数据会出现此信号
}
...
void processpool<T>::run_child()
{
    ...
    users = NULL;
    close(pipefd);
    //close(m_listenfd); /*我们将这句话注释掉，以提醒读者，应该有m_listenfd的创建者来关闭这个文件描述符，
                ...
                else
                {
                    for(int i = 0; i < ret; ++i)
                    {
                        switch(signals[i])
                        {
                            ...
                            case SIGINT:
                                {
                                    /*如果父进程接收到终止信号，那么就杀死所有子进程，并等待他们全部结束，当然，
