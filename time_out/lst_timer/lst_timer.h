...
public:
    ...
    client_data *user_data; /*回调函数处理的客户数据，由定时器的执行者专递给回调函数*/
...
public:
    ...
    void del_timer(util_timer *timer)
    {
         ...
         * 则将链表的尾结点重置为原尾结点的前一个节点,然后删除目标定时器*/
        if(timer == tail)
        {
            tail = tail->prev;
            tail->next = NULL;
...
private:
    /*一个重载的辅助函数，它被公有的add_timer函数和adjust_timer函数调用，该函数表示
    ...
    void add_timer(util_timer *timer, util_timer *lst_head)
    {
        ...
        /*遍历lit_head节点之后的部分链表，直到找到一个超时时间大于目标定时器的超时时间的节点，并将目标定时器插入该节点之前*/
        while(tmp)
        {
        ...
        /*如果遍历完lst_head节点之后的部分链表，仍未找到超时时间大于目标定时器
